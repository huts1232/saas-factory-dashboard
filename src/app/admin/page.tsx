...
import { Users, BarChart3, CreditCard, Activity, Database, Loader2, DollarSign, TrendingUp } from 'lucide-react'
...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    if (!authLoading && isAdmin) {
      ...
      ]).then(([proj, subRes, adminRes, credRes]) => {
  ...
  const mrr = paidSubs.reduce((s, sub) => s + (sub.plan === 'pro' ? 49 : sub.plan === 'starter' ? 19 : 0), 0)
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
        {[
          { label: 'Total Vaxes', value: projects.length, icon: <Database className="h-5 w-5 text-accent" />, sub: `${liveProjects.length} live, ${previewProjects.length} preview` },
          { label: 'Users', value: uniqueUserIds.length, icon: <Users className="h-5 w-5 text-blue-500" />, sub: `${paidSubs.length} paid` },
          { label: 'MRR', value: `$${mrr}`, icon: <DollarSign className="h-5 w-5 text-green-500" />, sub: `${paidSubs.length} subscriptions` },
      ...
      <div className="bg-bg-card border border-border-default rounded-xl overflow-hidden mb-8">
        <div className="px-6 py-4 border-b border-border-default">
          <h2 className="font-semibold">Users ({uniqueUserIds.length})</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {subs.map(s => {
                const userProjects = projects.filter(p => p.user_id === s.user_id)
                return (
                  <tr key={s.id} className="border-b border-border-default hover:bg-bg-elevated/50">
                    ...
                    <td className="px-6 py-3 text-xs text-text-primary">{userProjects.length} vaxes</td>
                    <td className="px-6 py-3 text-xs text-text-muted">{new Date(s.created_at).toLocaleDateString()}</td>
                  </tr>
                )
              })}
              {subs.length === 0 && (
                <tr><td colSpan={5} className="px-6 py-8 text-center text-text-muted">No users yet</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <div className="bg-bg-card border border-border-default rounded-xl overflow-hidden mb-8">
        <div className="px-6 py-4 border-b border-border-default">
          <h2 className="font-semibold">All Vaxes ({projects.length})</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
      ...
      <div className="bg-bg-card border border-border-default rounded-xl p-6">
        <h2 className="font-semibold mb-4 flex items-center gap-2"><TrendingUp className="h-5 w-5 text-green-500" /> Revenue</h2>
        <div className="grid grid-cols-3 gap-6">
          <div className="bg-bg-elevated rounded-xl p-4 text-center">
            <p className="text-2xl font-bold text-accent-green">${mrr}</p>
            <p className="text-xs text-text-muted mt-1">Monthly Recurring Revenue</p>
          </div>
          <div className="bg-bg-elevated rounded-xl p-4 text-center">
            <p className="text-2xl font-bold text-accent">{paidSubs.filter(s => s.plan === 'starter').length}</p>
            <p className="text-xs text-text-muted mt-1">Starter Plans ($19/mo)</p>
          </div>
          <div className="bg-bg-elevated rounded-xl p-4 text-center">
            <p className="text-2xl font-bold text-purple-400">{paidSubs.filter(s => s.plan === 'pro').length}</p>
            <p className="text-xs text-text-muted mt-1">Pro Plans ($49/mo)</p>
          </div>
        </div>
      </div>
    </div>
