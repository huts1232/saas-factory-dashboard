...
import { Loader2, Rocket, ExternalLink, Github, Play, Trash2, Lock, Settings, Link2 } from 'lucide-react'
...
export default function ProjectPage() {
  ...
  const isPreview = isFree && project.current_step <= 2 && isPending
  const hasArchitecture = !!project.architecture
  const progress = Math.round((project.current_step / 11) * 100)

  return (
    <div className={isBuilding || isLive ? 'pb-8' : 'pb-28'}>
      {/* ===== HEADER ===== */}
      <div className="px-8 pt-8 pb-4">
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-3">
              ...
              {isAdmin && <span className="text-[8px] px-1.5 py-0.5 rounded bg-accent-purple/20 text-accent-purple font-bold uppercase">Admin</span>}
            </div>
        ...
        {/* Metrics row */}
        {!isBuilding && project.total_tokens > 0 && (
          <div className="flex gap-6 mt-3 text-xs text-text-muted">
            ...
            <span>Credits: <span className="text-text-primary font-mono">~{project.current_step}</span></span>
          </div>
        )}
      </div>
          ...
          <div className="relative p-6 flex items-center gap-5">
            ...
            <div>
              <p className="text-base font-semibold text-text-primary">
                {project.product_name || 'Your SaaS'} is being built...
              </p>
              <p className="text-sm text-text-muted mt-0.5">Usually takes 5-10 minutes. You can leave this page and come back.</p>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="mx-8 mb-6 relative overflow-hidden rounded-2xl border border-accent/30">
          ...
          <div className="relative p-8 text-center">
            ...
            <p className="text-sm text-text-secondary mb-2 max-w-md mx-auto">
              Connect your tools and deploy this SaaS on your own accounts.
            </p>
      ...
      {isFailed && (
        <div className="mx-8 mb-6 bg-accent-pink/5 border border-accent-pink/20 rounded-xl p-4 flex items-center justify-between">
          ...
          <GlowButton variant="accent" size="sm" onClick={handleContinueBuild} loading={deploying}><Play className="h-3 w-3" /> Retry</GlowButton>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 px-8">
        ...
        <div className="lg:col-span-2">
          <div className="flex gap-1 mb-4 bg-bg-secondary rounded-lg p-1 w-fit">
            {(['overview', ...(hasArchitecture ? ['preview'] : []), ...(isBuilding || isLive ? ['logs'] : [])] as const).map(t => (
          ...
          {tab === 'overview' && (
            <div className="space-y-4">
              ...
              {/* Post-build config for live projects */}
              {isLive && !isFree && (
                <div className="bg-bg-card border border-border-default rounded-xl p-5">
                  ...
                  <div className="space-y-3">
                    {[
                      ...
                      { icon: '🔐', name: 'Google Auth', desc: 'OAuth credentials for this app' },
                      { icon: '🌐', name: 'Custom Domain', desc: 'Connect your own .com' },
                      { icon: '📧', name: 'Email (Resend)', desc: 'Transactional emails for this app' },
                    ].map((cfg) => (
                  ...
                  <p className="text-[10px] text-text-muted mt-3 pt-3 border-t border-border-default">
                    These are configured per app. Each SaaS you build can have different accounts.
                  </p>
                </div>
              )}
      ...
      {/* ===== CHAT BAR — only when not building ===== */}
      {!isBuilding && (isLive || isPreview || isFailed || isPending) && (
